...
public final class WorkerControl {
    ...
    public boolean acquireLock(UUID controllerId) {
        logger.info("Controller WorkerControl Acquiring lock of worker <" + metadata.getWorkerId() + ">");
        String lockUri = getUrlPrefix() + "/worker/control/acquireLock/" + controllerId;
        System.out.println("WorkerControl acquireLock UUID:"+controllerId);
        System.out.println("WorkerControl acquireLock urlprefix:"+getUrlPrefix());
        boolean locked = tellWorker(httpClient.preparePost(lockUri), Response.Status.NO_CONTENT);
        if (locked) {
            logger.info("Controller WorkerControl Successfully acquired lock of worker <" + metadata.getWorkerId() + ">");
        } else {
            logger.warn("Controller WorkerControl Acquiring lock of worker <" + metadata.getWorkerId() + "> failed");
        }
